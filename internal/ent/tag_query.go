...
func (tq *TagQuery) QueryBooks() *BookQuery {
	query := (&BookClient{config: tq.config}).Query()
...
func (tq *TagQuery) FirstID(ctx context.Context) (id ksuid.ID, err error) {
	var ids []ksuid.ID
...
func (tq *TagQuery) FirstIDX(ctx context.Context) ksuid.ID {
...
func (tq *TagQuery) OnlyID(ctx context.Context) (id ksuid.ID, err error) {
	var ids []ksuid.ID
...
func (tq *TagQuery) OnlyIDX(ctx context.Context) ksuid.ID {
...
func (tq *TagQuery) IDs(ctx context.Context) (ids []ksuid.ID, err error) {
...
func (tq *TagQuery) IDsX(ctx context.Context) []ksuid.ID {
	ids, err := tq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (tq *TagQuery) WithBooks(opts ...func(*BookQuery)) *TagQuery {
	query := (&BookClient{config: tq.config}).Query()
...
func (tq *TagQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tag, error) {
	...
	if query := tq.withBooks; query != nil {
		if err := tq.loadBooks(ctx, query, nodes,
			func(n *Tag) { n.Edges.Books = []*Book{} },
			func(n *Tag, e *Book) { n.Edges.Books = append(n.Edges.Books, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range tq.withNamedBooks {
		if err := tq.loadBooks(ctx, query, nodes,
			func(n *Tag) { n.appendNamedBooks(name) },
			func(n *Tag, e *Book) { n.appendNamedBooks(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (tq *TagQuery) WithNamedBooks(name string, opts ...func(*BookQuery)) *TagQuery {
	query := (&BookClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if tq.withNamedBooks == nil {
		tq.withNamedBooks = make(map[string]*BookQuery)
	}
	tq.withNamedBooks[name] = query
	return tq
}
