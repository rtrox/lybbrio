...
func (lq *LanguageQuery) QueryBooks() *BookQuery {
...
func (lq *LanguageQuery) FirstID(ctx context.Context) (id ksuid.ID, err error) {
	var ids []ksuid.ID
...
func (lq *LanguageQuery) FirstIDX(ctx context.Context) ksuid.ID {
...
func (lq *LanguageQuery) OnlyID(ctx context.Context) (id ksuid.ID, err error) {
	var ids []ksuid.ID
...
func (lq *LanguageQuery) OnlyIDX(ctx context.Context) ksuid.ID {
...
func (lq *LanguageQuery) IDs(ctx context.Context) (ids []ksuid.ID, err error) {
...
func (lq *LanguageQuery) IDsX(ctx context.Context) []ksuid.ID {
	ids, err := lq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (lq *LanguageQuery) WithBooks(opts ...func(*BookQuery)) *LanguageQuery {
	query := (&BookClient{config: lq.config}).Query()
...
func (lq *LanguageQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Language, error) {
	...
	if query := lq.withBooks; query != nil {
		if err := lq.loadBooks(ctx, query, nodes,
			func(n *Language) { n.Edges.Books = []*Book{} },
			func(n *Language, e *Book) { n.Edges.Books = append(n.Edges.Books, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range lq.withNamedBooks {
		if err := lq.loadBooks(ctx, query, nodes,
			func(n *Language) { n.appendNamedBooks(name) },
			func(n *Language, e *Book) { n.appendNamedBooks(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (lq *LanguageQuery) loadBooks(ctx context.Context, query *BookQuery, nodes []*Language, init func(*Language), assign func(*Language, *Book)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[ksuid.ID]*Language)
	for i := range nodes {
	...
	query.Where(predicate.Book(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(language.BooksColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.language_books
		if fk == nil {
			return fmt.Errorf(`foreign-key "language_books" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "language_books" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (lq *LanguageQuery) WithNamedBooks(name string, opts ...func(*BookQuery)) *LanguageQuery {
	query := (&BookClient{config: lq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
