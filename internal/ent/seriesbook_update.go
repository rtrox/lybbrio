...
func (sbu *SeriesBookUpdate) sqlSave(ctx context.Context) (n int, err error) {
	...
	_spec := sqlgraph.NewUpdateSpec(seriesbook.Table, seriesbook.Columns, sqlgraph.NewFieldSpec(seriesbook.FieldSeriesID, field.TypeString), sqlgraph.NewFieldSpec(seriesbook.FieldBookID, field.TypeString))
...
func (sbuo *SeriesBookUpdateOne) sqlSave(ctx context.Context) (_node *SeriesBook, err error) {
	...
	_spec := sqlgraph.NewUpdateSpec(seriesbook.Table, seriesbook.Columns, sqlgraph.NewFieldSpec(seriesbook.FieldSeriesID, field.TypeString), sqlgraph.NewFieldSpec(seriesbook.FieldBookID, field.TypeString))
	if id, ok := sbuo.mutation.SeriesID(); !ok {
		return nil, &ValidationError{Name: "series_id", err: errors.New(`ent: missing "SeriesBook.series_id" for update`)}
	} else {
		_spec.Node.CompositeID[0].Value = id
	}
	if id, ok := sbuo.mutation.BookID(); !ok {
		return nil, &ValidationError{Name: "book_id", err: errors.New(`ent: missing "SeriesBook.book_id" for update`)}
	} else {
