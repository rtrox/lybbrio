...
// BookCovers is the resolver for the bookCovers field.
func (r *queryResolver) BookCovers(ctx context.Context, after *entgql.Cursor[ksuid.ID], first *int, before *entgql.Cursor[ksuid.ID], last *int, orderBy *ent.BookCoverOrder, where *ent.BookCoverWhereInput) (*ent.BookCoverConnection, error) {
	return r.client.BookCover.Query().
		Paginate(ctx, after, first, before, last,
			ent.WithBookCoverOrder(orderBy),
			ent.WithBookCoverFilter(where.Filter),
		)
}
